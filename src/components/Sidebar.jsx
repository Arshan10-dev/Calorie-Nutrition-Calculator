  ...
  return (
    <aside className="hidden md:flex flex-col w-60 h-full bg-white/60 dark:bg-gray-900/60 backdrop-blur-xl border-r border-gray-200/60 dark:border-gray-800/60 pt-16">
      ...
      <div className="px-4 py-4 border-t border-gray-200/60 dark:border-gray-800/60">
        <p className="text-xs text-gray-400 dark:text-gray-600 text-center">NutriIQ v1.0 · Built for health</p>
      </div>
    </aside>
  );
}
        ...
        <>
          ...
          >
            <div className="flex items-center justify-between px-4 h-16 border-b border-gray-200 dark:border-gray-800">
              ...
              <button onClick={() => setSidebarOpen(false)} className="p-1.5 rounded-lg text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                <X size={18} />
              </button>
            </div>
            <div className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
              {NAV_ITEMS.map(item => (
                <NavItem
                  key={item.id} item={item} active={activeView === item.id}
                  onClick={id => { setActiveView(id); setSidebarOpen(false); }}
                />
              ))}
            </div>
          </motion.aside>
