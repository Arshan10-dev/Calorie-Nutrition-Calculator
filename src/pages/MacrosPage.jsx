...
const MACRO_INFO = [
  { key: 'protein', label: 'Protein',       icon: '🥩', color: 'sky',    description: 'Essential for muscle repair and growth. Each gram provides 4 kcal.', foods: ['Chicken breast','Eggs','Greek yogurt','Lentils','Tuna'] },
  { key: 'fat',     label: 'Fat',            icon: '🥑', color: 'orange', description: 'Vital for hormones, brain function and fat-soluble vitamins. 9 kcal/g.', foods: ['Avocado','Olive oil','Nuts','Salmon','Cheese'] },
  { key: 'carbs',   label: 'Carbohydrates',  icon: '🍠', color: 'violet', description: 'Primary energy source for the body and brain. 4 kcal per gram.', foods: ['Oats','Sweet potato','Brown rice','Banana','Beans'] },
];
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Your daily macro targets based on goal and calorie needs</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {MACRO_INFO.map((info, i) => (
          <motion.div key={info.key} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
          ...
          >
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="text-right">
                <p className="text-xl font-display font-bold font-mono">{formatNumber(macros?.[info.key]?.grams)}g</p>
                <p className="text-xs opacity-70">{formatNumber(macros?.[info.key]?.calories)} kcal · {macros?.[info.key]?.percent}%</p>
              </div>
            </div>
            <p className="text-xs opacity-80 mb-3 leading-relaxed">{info.description}</p>
            <div className="flex flex-wrap gap-1.5">
              {info.foods.map(f => (
                <span key={f} className="text-xs px-2 py-0.5 rounded-full bg-white/50 dark:bg-white/10 font-medium">{f}</span>
              ))}
            </div>
          </motion.div>
        ))}
