  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="h-2.5 rounded-full bg-gray-100 dark:bg-gray-700 overflow-hidden">
        <motion.div className={`h-full rounded-full ${color}`} initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 1, ease: [0.22, 1, 0.36, 1] }} />
      </div>
    </div>
  );
}
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Personalised daily water intake recommendations</p>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="bg-white dark:bg-gray-800/60 rounded-2xl border border-gray-200/80 dark:border-gray-700/60 p-5 shadow-card space-y-5">
          ...
          <div className="space-y-3 pt-2">
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Hydration levels throughout the day</p>
            {hydrationLevels.map(h => <HydrationLevel key={h.label} {...h} />)}
          </div>
        </div>
      </div>
      <div className="bg-white dark:bg-gray-800/60 rounded-2xl border border-gray-200/80 dark:border-gray-700/60 p-5 shadow-card">
        ...
        <div className="mt-4 flex items-start gap-2 p-3 bg-sky-50 dark:bg-sky-900/10 rounded-xl border border-sky-100 dark:border-sky-800/30">
          ...
          <p className="text-xs text-sky-700 dark:text-sky-300">Formula: 35 ml × body weight (kg) + activity bonus. Thirst and urine colour are better real-time indicators.</p>
        </div>
      </div>
    </motion.div>
  );
