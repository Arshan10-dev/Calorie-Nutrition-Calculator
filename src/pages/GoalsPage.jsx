...
export default function GoalsPage() {
  ...
  // Compute simple start weight (assume 5% above/below for demo)
  const direction = goalWeight < currentWeight ? 'lose' : 'gain';
  const startWeight = direction === 'lose'
    ? currentWeight * 1.05
    : currentWeight * 0.95;
    ...
    >
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
          Set your target, track your timeline
        </p>
      </div>
            ...
            <>
              ...
              <div className="space-y-2">
                ...
                {[
                  ...
                  >
                    <span className={`font-medium ${c.isActive ? 'text-brand-700 dark:text-brand-300' : 'text-gray-600 dark:text-gray-400'}`}>
                      {c.label}
                    </span>
                    <span className={`font-mono font-bold text-xs ${c.isActive ? 'text-brand-600 dark:text-brand-300' : 'text-gray-500 dark:text-gray-400'}`}>
                      {formatNumber(c.value)} kcal
                    </span>
                  </div>
                ))}
              </div>
      ...
      {isValid && goalWeight ? (
        <div className="bg-white dark:bg-gray-800/60 rounded-2xl border border-gray-200/80 dark:border-gray-700/60 p-5 shadow-card">
          ...
          <p className="text-xs text-gray-400 dark:text-gray-500 mb-4">
            Projected at 0.5 kg/week — a safe, sustainable rate
          </p>
          <WeightProgressChart />
        </div>
      ) : null}
