...
function MetricBlock({ icon: Icon, label, value, unit, color, note, large }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 16 }}
      animate={{ opacity: 1, y: 0 }}
      className={`bg-white dark:bg-gray-800/60 rounded-2xl border border-gray-200/80 dark:border-gray-700/60 p-5 shadow-card ${large ? 'xl:col-span-2' : ''}`}
    >
  ...
  return (
    ...
    >
      ...
      <div className="bg-gradient-to-r from-violet-50 to-purple-50 dark:from-violet-900/10 dark:to-purple-900/10 border border-violet-100 dark:border-violet-800/30 rounded-2xl p-4">
        <p className="text-xs font-semibold text-violet-600 dark:text-violet-400 mb-1.5 uppercase tracking-wide">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <MetricBlock
          ...
          label="BMR"
          value={bmr}
          unit="kcal/day at complete rest"
          color="bg-gradient-to-br from-violet-400 to-purple-600"
          note="Energy needed to sustain basic bodily functions"
        />
        <MetricBlock
          icon={Flame}
          label="TDEE"
          value={tdee}
          unit="total daily energy expenditure"
          color="bg-gradient-to-br from-ember-400 to-rose-600"
          note="BMR adjusted for your activity level — eat this to maintain weight"
        />
        <MetricBlock
          icon={TrendingDown}
          label="Lose Weight"
          value={calorieTargets.lose}
          unit="kcal/day (−500 deficit)"
          color="bg-gradient-to-br from-sky-400 to-blue-600"
          note="Targets ~0.5 kg/week fat loss"
        />
        <MetricBlock
          icon={TrendingUp}
          label="Gain Muscle"
          value={calorieTargets.gain}
          unit="kcal/day (+300 surplus)"
          color="bg-gradient-to-br from-ember-400 to-orange-600"
          note="Lean bulk — minimise fat gain"
        />
      </div>
